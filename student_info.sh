...
FROM students 
WHERE last_name ILIKE '%sa%' OR last_name ILIKE '%r_'"
)"
...
echo "$($PSQL "SELECT ROUND(AVG(gpa), 2) 
FROM students"
)"
...
echo "$($PSQL "SELECT DISTINCT(course) 
FROM students 
...
echo "$($PSQL "SELECT course FROM students FULL JOIN majors USING(major_id) 
FULL JOIN majors_courses USING(major_id) 
FULL JOIN courses USING(course_id) 
